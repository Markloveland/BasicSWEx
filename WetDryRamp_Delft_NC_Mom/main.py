...
Q      =   as_vector((h_wd, h_wd*u[1], h_wd*u[2]))
Q_flux =   as_vector((h_wd, h_wd*u[1],h_wd*u[2]))
...
eta = h-h_b
g_vec = as_vector((0,
 					-ux/h_wd*(qx.dx(0) + qy.dx(1)) - g*h_b_wd.dx(0),
 					-uy/h_wd*(qx.dx(0) + qy.dx(1)) - g*h_b_wd.dx(1)))
...
p_body = as_vector((p[0],p[1]/h_wd,p[2]/h_wd))
#start adding to residual, beggining with body term
F = -inner(Fu,grad(p_body))*dx
#add RHS forcing
F += inner(S,p)*dx
...
for marker, func in boundaries:
	if marker == 1:
		...
		F += dot(dot(Fu, n), p_body) * ds(marker)
	elif marker == 2:
		#this is the wall condition, no flux on this part
		F += dot(dot(Fu_wall, n), p_body)*ds(marker)
...
flux = dot(avg(Fu), n('+')) + 0.5*C*jump(Q_flux)

F += inner(flux, jump(p_body))*dS
