...
class CustomNewtonProblem:
    ...
    def __init__(self,F,u,bc,comm,solver_parameters={},lines=[],mesh=None):
...
class LinePreconditioner:
    ...
    def __init__(self, A, mesh,lines):
        ...
        mat.createAIJ((A.size[0], A.size[0]), nnz=np.full(A.size[0], A.size[0], dtype=np.int32), comm=mesh.comm)
        mat.setUp()
    ...
    def precondition(self, rhs):
        ...
        #print("current blocksize!!!",self.block_size)
        ...
        #viewer_obj.createASCII("A_pre_permute.txt")
        #viewer_obj.pushFormat(9)
        #viewer_obj.view(obj=self.A)
        #plt.close()
        ...
        #viewer_obj = PETSc.Viewer()
        #viewer_obj.createASCII("A_permuted.txt")
        #viewer_obj.pushFormat(9)
